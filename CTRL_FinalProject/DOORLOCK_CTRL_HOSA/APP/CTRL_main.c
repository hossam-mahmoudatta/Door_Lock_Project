...
int main(void) {
	...
	while (1) {
		if (ackSent) {
			// Acknowledgment sent successfullyy

			// Save received data into variables
			receivedPassword1 		= (uint8*) dataReceived1;
			receivedPasswordVerif 	= (uint8*) dataReceived2;

			// Perform desired operations
			displayPasswords(receivedPassword1, receivedPasswordVerif);

			// Reset received data for the next iterationn
			dataReceived1 = 0;
			dataReceived2 = 0;
		}
	}
